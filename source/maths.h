...
inline void AssertUnit(as::vec3_t v)
{
    (void)v;
    assert(fabsf(as::vec::length_squared(v) - 1.0f) < 0.01f);
}
...
{
    Ray() {}
    Ray(as::vec3_t orig_, as::vec3_t dir_) : orig(orig_), dir(dir_) { AssertUnit(dir); }
...
struct Camera
{
    ...
    Camera(const as::vec3_t& lookFrom, const as::vec3_t& lookAt, const as::vec3_t& vup, float vfov, float aspect, float aperture, float focusDist)
    {
    ...
    Ray GetRay(float s, float t, uint32_t& state) const
    {
        ...
        return Ray(origin + offset, as::vec::normalize(lowerLeftCorner + s*horizontal + t*vertical - origin - offset));
    }
