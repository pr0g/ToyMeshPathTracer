...
vec_t<T, n> operator*(const vec_t<T, n> v, const mat_t<T, r, c>& mat)
#elif defined AS_COL_MAJOR
vec_t<T, n> operator*(const mat_t<T, r, c>& mat, const vec_t<T, n> v)
#endif // AS_ROW_MAJOR ? AS_COL_MAJOR
{
#if defined AS_ROW_MAJOR
...
template<typename T, size_t r, size_t c>
mat_t<T, r, c> operator*(const mat_t<T, r, c>& mat, T scalar)
{
    mat_t<T, r, c> result;
    for (size_t rowIndex = 0; rowIndex < r; ++rowIndex) {
        for (size_t colIndex = 0; colIndex < c; ++colIndex) {
...
template<typename T, size_t r, size_t c>
void operator*=(mat_t<T, r, c>& mat, T scalar)
{
    for (size_t rowIndex = 0; rowIndex < r; ++rowIndex) {
        for (size_t colIndex = 0; colIndex < c; ++colIndex) {
