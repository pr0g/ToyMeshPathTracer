...
int main(int argc, const char** argv)
{
    ...
    as::vec3_t lookat = sceneCenter + sceneSize * as::vec3_t(0,-0.1f,0);
    float distToFocus = as::vec::length(lookfrom - lookat);
    float aperture = 0.03f;
    auto camera = Camera(lookfrom, lookat, as::vec3_t(0, 1, 0), 60, float(screenWidth) / float(screenHeight), aperture, distToFocus);
